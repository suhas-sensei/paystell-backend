...
describe('WalletVerificationController', () => {
    ...
    describe('POST /wallet-verification/initiate', () => {
        it('should initiate wallet verification', async () => {
            (WalletVerificationService.prototype.initiateVerification as jest.Mock).mockResolvedValue({
        ...
        it('should return 400 for invalid wallet address', async () => {
            (WalletVerificationService.prototype.initiateVerification as jest.Mock).mockRejectedValue(new Error('Invalid Stellar wallet address.'));
            ...
            expect(response.body).toHaveProperty('message', 'Invalid Stellar wallet address.');
        });
    });

    describe('POST /wallet-verification/verify', () => {
        it('should verify wallet', async () => {
            (WalletVerificationService.prototype.verifyWallet as jest.Mock).mockResolvedValue(true);
