...
describe("WebhookService", () => {
  ...
  describe("getMerchantWebhook", () => {
    it("should return a merchant webhook if it is active", async () => {
      jest
        .spyOn(webhookService, "getMerchantWebhook")
        .mockResolvedValue(mockMerchantWebhook);
    ...
    it("should throw an error if webhook is inactive", async () => {
      jest
        .spyOn(webhookService, "getMerchantWebhook")
        .mockImplementation(async (_merchantId: string) => {
      ...
      await expect(
        webhookService.getMerchantWebhook(mockMerchant.id),
      ).rejects.toThrow("Merchant web hook not found");
    });
  });
