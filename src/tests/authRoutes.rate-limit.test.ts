...
describe("Auth Routes with Rate Limiting", () => {
  ...
  beforeEach(() => {
    ...
    authController = new (AuthController as jest.MockedClass<
      typeof AuthController
    >)() as unknown as MockAuthController;
  ...
  it("should apply 2FA rate limiting correctly", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await request(app).post("/auth/login-2fa").send({
    ...
    const response = await request(app).post("/auth/login-2fa").send({
      email: "test@example.com",
      password: "password",
      token: "invalid",
    });
