...
export class SentryLogger implements ExternalLoggerAdapter {
  ...
  public initialize(config: { dsn: string; environment?: string }): void {
    try {
      ...
      import("@sentry/node")
        ...
        .catch((err) => {
          logger.error(
            "Failed to initialize Sentry. Is @sentry/node installed?",
            { error: err.message },
          );
        });
    } catch (error) {
  ...
  public warn(message: string, metadata?: LogMetadata): void {
    ...
    try {
      ...
      logger.warn("Failed to log warning to Sentry", {
        error: (error as Error).message,
      });
    }
  }
  ...
  public info(message: string, metadata?: LogMetadata): void {
    ...
    try {
      ...
      logger.info("Failed to log info to Sentry", {
        error: (error as Error).message,
      });
    }
  }
  ...
  public debug(message: string, metadata?: LogMetadata): void {
    ...
    try {
      ...
      logger.debug("Failed to log debug to Sentry", {
        error: (error as Error).message,
      });
    }
  }
  ...
  public initialize(config: {
    ...
    try {
      ...
      import("dd-trace")
        ...
        .catch((err) => {
          logger.error("Failed to initialize DataDog. Is dd-trace installed?", {
            error: err.message,
          });
        });
    } catch (error) {
      logger.error("Failed to initialize DataDog", {
        error: (error as Error).message,
      });
    }
  }
